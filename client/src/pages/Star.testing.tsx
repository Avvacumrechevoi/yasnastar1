...
export function StarPointLabelOverlay({
  pointIndex,
  labelLines,
  tooltip,
  isSelected,
...
export function getPointLabelBadgeGapMetrics(index: number, text?: string | null) {
  const point = polarPoint(index, POINT_RADIUS);
  const layout = getPointLabelLayout(point, text);
  const box = getLabelBox(layout, point);
  const badge = getPointNumberBadgePosition(point);
...
export function getMechanicButtonLabel(mechanic: Pick<Mechanic, "title" | "shortTitle">) {
...
export function StarMechanicListButton({ mechanic, isActive, onClick, tooltip }: StarMechanicListButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export function getMechanicVisualStyle(mechanic: Pick<Mechanic, "id" | "kind">) {
  if (mechanic.kind === "contrast") {
    return {
      haloWidth: 12,
      coreWidth: 4.4,
...
type StarContrastOverlayProps = {
  ...
  selectedPoint: number | null;
  starPoints: Point[];
};export function StarContrastOverlay({ mechanic, selectedPoint, starPoints }: StarContrastOverlayProps) {
  if (selectedPoint === null) {
    return null;
  }
...
function StarSelectionInsightCard({ model, className }: StarSelectionInsightCardProps) {
  ...
  const lessonBlock = model.blocks.find((block) => block.kind === "yasna") ?? null;
    ...
    >
      <div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="min-w-0 rounded-[18px] border border-white/8 bg-white/[0.035] px-3 py-3 text-left lg:w-[320px] lg:max-w-[320px] lg:flex-none">
          ...
          <p className="mt-1.5 break-words text-[12px] leading-[1.5] text-white/60">{lessonBlock?.description || model.headerDescription}</p>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2.5">
        {mechanicBlocks.length > 0 ? (
          mechanicBlocks.map((block) => (
            ...
            >
              <div className="text-[10px] uppercase tracking-[0.28em] text-[#8ab79f]">{block.title}</div>
          ...
          <section className={`flex ${STAR_SCENE_MIN} ${CENTER_COLUMN_DESKTOP_LAYOUT} flex-col overflow-hidden rounded-[34px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.025))] p-1.5 shadow-[0_30px_80px_rgba(0,0,0,0.24)] backdrop-blur-xl sm:p-2`}>
            <div className="mb-3">
              ...
              {contextPanelModel.blocks.length > 0 ? (
                <div
                  className="pointer-events-none absolute inset-x-3 bottom-3 z-[9] flex justify-center sm:inset-x-4 sm:bottom-4"
                  data-testid="star-selection-overlay"
                >
                  <StarSelectionInsightCard
                    model={contextPanelModel}
                    className="pointer-events-auto w-full max-w-[1120px] max-h-[min(42vh,28rem)] overflow-y-auto rounded-[28px] border-white/12 bg-[linear-gradient(180deg,rgba(7,22,15,0.92),rgba(4,13,9,0.95))] shadow-[0_28px_70px_rgba(0,0,0,0.34)] ring-1 ring-black/18 backdrop-blur-2xl"
                  />
                </div>
              ) : null}
            </div>
